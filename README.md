...
3. 添加以下资源地址：
   ...
   ```text
   /local/climate-sleep-curve-card.js?v=0.5.0
   ```

4. 资源类型选择 **JavaScript 模块**。
5. 刷新浏览器页面。

`/config/www/` 会映射为 `/local/`。升级文件后可以修改查询参数，例如从 `v=0.4.1` 改为 `v=0.5.0`，以绕过浏览器缓存。
...
自动启动时间使用 Home Assistant 原生风格的时间选择器；星期使用周一至周日的勾选项，不再需要填写数字。时间使用 Home Assistant 的本地时区。“自然结束后关闭空调”会在会话启动时形成快照，只对本次正常运行到末尾生效。删除控制器会停止它正在运行的会话，但不会关闭空调，也不会删除仍可由其他控制器使用的曲线。
...
点击“停止”只会取消尚未执行的节点，不会关闭空调，也不会恢复会话开始前的温度。
...
- 卡片不直接提供设备开关或 HVAC 模式操作；控制器设置只把“自然结束后关机”选择交给后端。
- 启动会话并不等于打开空调。
...
当前版本为 `0.5.0`，采用 [MIT License](LICENSE)。完整功能需要 Climate Sleep Curve 后端 `0.5.0` 或更高版本。
