...
`/config/www/` 会映射为 `/local/`。升级文件后可以修改查询参数，例如从 `v=0.1.0` 改为 `v=0.1.1`，以绕过浏览器缓存。
...
星期使用 `0,1,2,3,4,5,6` 表示周一到周日。例如工作日填写：

```text
0,1,2,3,4
```

时间使用 Home Assistant 的本地时区。删除控制器会停止它正在运行的会话，但不会关闭空调，也不会删除仍可由其他控制器使用的曲线。

## 运行会话

### 启动曲线

先通过原空调界面或设备遥控器打开空调，然后点击“启动曲线”。后端会立即处理第 0 分钟节点，并安排后续节点。
...
卡片只列出实体 ID 以 `climate.` 开头的当前状态实体。先确认空调集成已正常加载，并能在 Home Assistant 中手动设置目标温度。
...
更新时替换 `climate-sleep-curve-card.js`，并刷新浏览器缓存。前后端协议发生变化时应配套升级两个仓库。
...
当前版本为 `0.1.0`，采用 [MIT License](LICENSE)。发布版本应与兼容的 Climate Sleep Curve 后端一起验证。
