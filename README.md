...
- 配置按时间和星期自动启动。
- 配置互斥的结束动作：安全恢复启动前的目标温度和风速，或在曲线内部定时关闭空调。
- 启动、停止和重新开始会话。
...
- 是否在曲线内部定时关闭所选空调，以及启动后的关机时间（默认关闭）。
...
- “定时关闭空调”从曲线启动时计时，只允许选择严格位于曲线内部的时间，界面按 0.5 小时递增。例如 8 小时曲线可选 0.5～7.5 小时，设置 6 小时后，到点会自然结束会话、关闭空调，并取消 6 小时之后的曲线节点。明确停止不会关机。
...
- 当前使用的曲线名称。
- 本次会话选择的结束动作：安全恢复或定时关机；定时关机会显示相对时长和具体时刻。
...
- 卡片不直接提供设备开关或 HVAC 模式操作；控制器设置只把互斥的安全恢复/定时关机选择交给后端。
- 启动会话并不等于打开空调。
- 空调关闭、不可用或未知时，后端会跳过节点。
- 停止、重新开始或删除控制器不会关闭空调。
- 只有明确启用开关且会话在 Home Assistant 持续运行期间自然到达定时关机点时，后端才会调用关机；Home Assistant 重启恢复不会补关机。
...
手动更新时，从 [GitHub Releases](https://github.com/eggleader-zhang/climate-sleep-curve-card/releases) 下载最新的 `climate-sleep-curve-card.js`，替换 `/config/www/` 中的旧文件，并修改资源 URL 的 `?v=` 参数或强制刷新浏览器缓存。定时关机要求后端和卡片均为 `0.7.0` 或更高版本，前后端应配套升级。
